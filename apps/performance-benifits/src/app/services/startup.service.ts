import { FILTER_TYPE, GLOBAL_PARAM_KEY, SESSION_STORAGE_DATA_KEY } from '../app-constants';
...
export class StartUpService {
  ...
  getOtherMasterData(url: string, domainName: string) {
    return new Promise((resolve, reject) => {
      this._serverApi.get<any>(url).subscribe(resp => {
        if (resp) {
          let formattedRespWithDomainName:any = {};
  ...
  setUserAccessibleContractors(){
      let userAccessibleContractors:any = [];
      ...
      userInfo?.user_access_details.forEach((element:any) => {
 ...
 setUserAccessiblePromoterOrg(){
      ...
      orgs = userInfo?.user_access_details.map((d:any)=>{
        let promoterPrefix = allOrg.find((a:any)=>a.orgId == d.org_id)?.swaOrgPrefix;
        return {
          orgId: d.org_id,
          name: d.org_name,
  ...
  setUserAccessibleWorkStream(){
    let userAccessibleWorkStream:any = [];
    let userInfo: any = this._appRepoService.appRepo.masterDataByKey.get(MASTER_DATA.USER_INFO);
