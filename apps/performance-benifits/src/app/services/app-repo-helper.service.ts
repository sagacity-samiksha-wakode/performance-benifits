...
export class AppRepoHelperService {
  ...
  getMDataByName(list: any, name: string) {
    return list.find((d:any) => d.name.indexOf(name) > 0);
  }
  ...
  getWorkIdentifiersByOrg(orgId:any, worksteamId:any){
  ...
  getGlobalPersistantData(key:any){
    return this._appRepoService.appRepo.appGlobalPersistantData.get(key);
  }

  setGlobalPersistantData(key:any, value:any){
